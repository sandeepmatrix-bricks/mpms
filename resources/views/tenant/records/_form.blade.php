        ...
        <form method="POST" action="{{ $action }}">
          ...
          <div class="mb-3">
            ...
            <select name="collection_id" class="form-select @error('collection_id') is-invalid @enderror" required>
              <option value="">&mdash; Select collection &mdash;</option>
              @foreach ($collections as $c)
                <option value="{{ $c->id }}" @selected(old('collection_id', $record->collection_id) === $c->id)>{{ $c->name }}</option>
              @endforeach
            </select>
          ...
          <div class="mt-3">
            ...
            <a href="{{ route('tenant.records.index', $tenant) }}" class="btn btn-light">Cancel</a>
          </div>
        </form>
      </div>
    </div>
