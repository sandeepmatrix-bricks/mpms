            ...
            <div class="col-md-6 mb-3">
              ...
              <select name="tenant_id" class="form-select @error('tenant_id') is-invalid @enderror" required>
                ...
                @foreach ($tenants as $t)
                  <option value="{{ $t->id }}" @selected(old('tenant_id', $page->tenant_id) === $t->id)>{{ $t->name }}</option>
                @endforeach
              </select>
            ...
            <div class="col-md-6 mb-3">
              ...
              <select name="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
                ...
                @foreach ($parents as $p)
                  @if (! $page->exists || $p->id !== $page->id)
                    <option value="{{ $p->id }}" @selected(old('parent_id', $page->parent_id) === $p->id)>{{ $p->tenant?->name }} · {{ $p->title }}</option>
                  @endif
                @endforeach
              </select>
              @error('parent_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
              <small class="text-muted">Must belong to the same tenant.</small>
            </div>
          </div>
